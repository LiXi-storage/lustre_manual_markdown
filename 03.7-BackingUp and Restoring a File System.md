...
The first time that `lustre_rsync` is run, the user must specify a set of parameters for the program to use. These parameters are described in the following table and in [the section called “ lustre_rsync”](dbdoclet.50438219_63667.html). On subsequent runs, these parameters are stored in the the status file, and only the name of the status file needs to be passed to `lustre_rsync`.

Before using `lustre_rsync`:

- Register the changelog user. For details, see the [*System Configuration Utilities*](systemconfigurationutilities.html)( `changelog_register`) parameter in the [*System Configuration Utilities*](systemconfigurationutilities.html)( `lctl`).
...
**Prior to Lustre software release 2.3**, the only successful way to perform an MDT backup and restore was to do a device-level backup as described in [the section called “ Backing Up and Restoring an MDT or OST (ldiskfs Device Level)”](dbdoclet.backup_device.html). The ability to do MDT file-level backups is not available for Lustre software release 2.0 through 2.2, because restoration of the Object Index (OI) file does not return the MDT to a functioning state.
...
The index_backup is also valid for an ldiskfs-based system, that will be used when migrating data between ldiskfs-based and zfs-based systems as described in [the section called “ Migration Between ZFS and ldiskfs Target Filesystems ”](migrate_backends.html).
...
9. **Unmount the file system.**
   ...
   When restoring an OST backup on a different node as part of an OST migration, you also have to change server NIDs and use the `--writeconf` command to re-generate the configuration logs. See [*Lustre Maintenance*](lustremaintenance.html)(Changing a Server NID).
...
11. Unmount the new file system.
    ...
    If the restored system has a different NID from the backup system, please change the NID. For detail, please refer to [the section called “ Changing a Server NID”](dbdoclet.changingservernid.html). For example:
...
Beginning with Lustre 2.11.0, it is possible to migrate between ZFS and ldiskfs backends. For migrating OSTs, it is best to use `lfs find`/`lfs_migrate` to empty out an OST while the filesystem is in use and then reformat it with the new fstype. For instructions on removing the OST, please see [the section called “Removing an OST from the File System”](section_remove_ost.html).

### Migrate from a ZFS to an ldiskfs based filesystem

The first step of the process is to make a ZFS backend backup using `tar` as described in [the section called “ Backing Up an OST or MDT (Backend File System Level)”](backup_fs_level.html).

Next, restore the backup to an ldiskfs-based system as described in [the section called “ Restoring a File-Level Backup”](backup_fs_level.restore.html).

### Migrate from an ldiskfs to a ZFS based filesystem

The first step of the process is to make an ldiskfs backend backup using `tar` as described in [the section called “ Backing Up an OST or MDT (Backend File System Level)”](backup_fs_level.html).
