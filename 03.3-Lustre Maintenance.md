...
- [the section called “ Working with Inactive OSTs”](dbdoclet.50438199_42877.html)
- [the section called “ Finding Nodes in the Lustre File System”](dbdoclet.50438199_15240.html)
- [the section called “ Mounting a Server Without Lustre Service”](dbdoclet.50438199_26070.html)
- [the section called “ Regenerating Lustre Configuration Logs”](dbdoclet.50438199_54623.html)
- [the section called “ Changing a Server NID”](dbdoclet.changingservernid.html)
- [the section called “Adding a New MDT to a Lustre File System”](dbdoclet.adding_new_mdt.html)
- [the section called “ Adding a New OST to a Lustre File System”](dbdoclet.adding_new_ost.html)
- [the section called “ Removing and Restoring MDTs and OSTs”](dbdoclet.deactivating_mdt_ost.html)
- [the section called “Removing an MDT from the File System”](dbdoclet.rmremotedir.html)
- [the section called “ Working with Inactive MDTs”](dbdoclet.inactivemdt.html)
- [the section called “Removing an OST from the File System”](section_remove_ost.html)
- [the section called “ Backing Up OST Configuration Files”](section_ydg_pgt_tl.html)
- [the section called “ Restoring OST Configuration Files”](section_restore_ost.html)
- [the section called “Returning a Deactivated OST to Service”](section_ucf_qgt_tl.html)
- [the section called “ Aborting Recovery”](dbdoclet.50438199_77819.html)
- [the section called “ Determining Which Machine is Serving an OST ”](dbdoclet.50438199_12607.html)
- [the section called “ Changing the Address of a Failover Node”](dbdoclet.50438199_62333.html)
- [the section called “ Separate a combined MGS/MDT”](dbdoclet.50438199_62545.html)
...
2. Balance OST space usage (possibly).
   ...
   See [the section called “ `lfs_migrate` ”](dbdoclet.lfs_migrate.html) for details.
...
- A hard drive has failed and a RAID resync/rebuild is underway, though the OST can also be marked *degraded* by the RAID system to avoid allocating new files on the slow OST which can reduce performance, see [the section called “ Handling Degraded OST RAID Arrays”](dbdoclet.degraded_ost.html) for more details.
- OST is nearing its space capacity, though the MDS will already try to avoid allocating new files on overly-full OSTs if possible, see [the section called “Allocating Free Space on OSTs”](dbdoclet.balancing_free_space.html) for details.
...
1. If the OST is functional, and there are files located on the OST that need to be migrated off of the OST, the file creation for that OST should be temporarily deactivated on the MDS (each MDS if running with multiple MDS nodes in DNE mode).
   ...
   1. Introduced in Lustre 2.9With Lustre 2.9 and later, the MDS should be set to only disable file creation on that OST by setting `max_create_count` to zero:`mds# lctl set_param osp.*osc_name*.max_create_count=0`This ensures that files deleted or migrated off of the OST will have their corresponding OST objects destroyed, and the space will be freed. For example, to disable `OST0000` in the filesystem `testfs`, run:`mds# lctl set_param osp.testfs-OST0000-osc-MDT*.max_create_count=0`on each MDS in the `testfs` filesystem.
...
3. Deactivate the OST.
   ...
   2. If there is not expected to be a replacement for this OST in the near future, permanently deactivate it on all clients and the MDS by running the following command on the MGS:
      ...
      A deactivated OST still appears in the file system configuration, though a replacement OST can be created using the `mkfs.lustre --replace` option, see [the section called “ Restoring OST Configuration Files”](section_restore_ost.html).
...
If the original OST is still available, it is best to follow the OST backup and restore procedure given in either [the section called “ Backing Up and Restoring an MDT or OST (ldiskfs Device Level)”](dbdoclet.backup_device.html), or [the section called “ Backing Up an OST or MDT (Backend File System Level)”](backup_fs_level.html) and [the section called “ Restoring a File-Level Backup”](backup_fs_level.restore.html).
...
For more information about the `--servicenode` and `--failnode` options, see [*Configuring Failover in a Lustre File System*](configuringfailover.html).

## Separate a combined MGS/MDT

These instructions assume the MGS node will be the same as the MDS node. For instructions on how to move MGS to a different node, see [the section called “ Changing a Server NID”](dbdoclet.changingservernid.html).
...
3. Copy the configuration data from MDT disk to the new MGS disk.
   ...
   See [the section called “ Regenerating Lustre Configuration Logs”](dbdoclet.50438199_54623.html) for alternative method.
