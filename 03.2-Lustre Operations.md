...
For unmount command syntax for a single OST, MDT, or MGT target please refer to [the section called “ Unmounting a Specific Target on a Server”](dbdoclet.umountTarget.html)
...
- When creating a file system, use mkfs.lustre. See [the section called “Setting Tunable Parameters with `mkfs.lustre`”](dbdoclet.50438194_17237.html)below.
- When a server is stopped, use tunefs.lustre. See [the section called “Setting Parameters with `tunefs.lustre`”](dbdoclet.50438194_55253.html)below.
- When the file system is running, use lctl to set or retrieve Lustre parameters. See [the section called “Setting Parameters with `lctl`”](dbdoclet.50438194_51490.html)and [the section called “Reporting Current Parameter Values”](dbdoclet.50438194_63247.html)below.
...
For more details about creating a file system,see [*Configuring a Lustre File System*](configuringlustre.html). For more details about `mkfs.lustre`, see [*System Configuration Utilities*](systemconfigurationutilities.html).
...
For more details about `tunefs.lustre`, see [*System Configuration Utilities*](systemconfigurationutilities.html).
...
To copy the contents of an existing OST to a new OST (or an old MDT to a new MDT), follow the process for either OST/MDT backups in [the section called “ Backing Up and Restoring an MDT or OST (ldiskfs Device Level)”](dbdoclet.backup_device.html)or [the section called “ Backing Up an OST or MDT (Backend File System Level)”](backup_fs_level.html). For more information on removing a MDT, see [the section called “Removing an MDT from the File System”](dbdoclet.rmremotedir.html).
